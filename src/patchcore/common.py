...
from patchcore.geometry import GeometryConfig, neighboring_bins
...
class GeometryNearestNeighbourScorer:
    """Nearest-neighbour scorer restricted to neighboring polar bins."""

    def __init__(self, n_nearest_neighbours=1, geometry_config=None, on_gpu=False, num_workers=4):
        self.n_nearest_neighbours = n_nearest_neighbours
    ...
    def fit(self, features, bin_ids):
        self.global_features = np.asarray(features, dtype=np.float32)
        self.bin_ids = np.asarray(bin_ids, dtype=np.int64)
        self.global_index = FaissNN(self.on_gpu, self.num_workers)
        self.global_index.fit(self.global_features)
        self.bin_features = {}
        self.bin_indexes = {}
        for bin_id in np.unique(self.bin_ids):
            values = self.global_features[self.bin_ids == bin_id]
            index = FaissNN(self.on_gpu, self.num_workers)
            index.fit(values)
            self.bin_features[int(bin_id)] = values
            self.bin_indexes[int(bin_id)] = index

    def _candidate_bins(self, bin_id, expand=1):
        return [b for b in neighboring_bins(bin_id, self.geometry_config, expand) if b in self.bin_indexes]

    def predict(self, query_features, query_bin_ids, valid_mask=None):
        queries = np.asarray(query_features, dtype=np.float32)
        bins = np.asarray(query_bin_ids, dtype=np.int64)
        valid = np.ones(len(queries), dtype=bool) if valid_mask is None else np.asarray(valid_mask, dtype=bool)
        scores = np.zeros(len(queries), dtype=np.float32)
        for bin_id in np.unique(bins[valid]):
            query_indices = np.flatnonzero(valid & (bins == bin_id))
            candidate_bins = self._candidate_bins(int(bin_id), 1)
            candidate_count = sum(len(self.bin_features[b]) for b in candidate_bins)
            if candidate_count < self.n_nearest_neighbours:
                candidate_bins = self._candidate_bins(int(bin_id), 2)
                candidate_count = sum(len(self.bin_features[b]) for b in candidate_bins)
            if candidate_count < self.n_nearest_neighbours or not candidate_bins:
                distances, _ = self.global_index.run(self.n_nearest_neighbours, queries[query_indices])
            else:
                all_distances = []
                for candidate_bin in candidate_bins:
                    k = min(self.n_nearest_neighbours, len(self.bin_features[candidate_bin]))
                    distances, _ = self.bin_indexes[candidate_bin].run(k, queries[query_indices])
                    all_distances.append(distances)
                distances = np.concatenate(all_distances, axis=1)
                distances.partition(min(self.n_nearest_neighbours, distances.shape[1]) - 1, axis=1)
                distances = np.sort(distances, axis=1)[:, : self.n_nearest_neighbours]
            scores[query_indices] = distances.mean(axis=1)
        return scores

    def save(self, save_folder, prepend=""):
        payload = {
            "features": self.global_features,
            "bin_ids": self.bin_ids,
            "geometry_config": self.geometry_config,
            "n_nearest_neighbours": self.n_nearest_neighbours,
        }
        with open(os.path.join(save_folder, prepend + "geometry_memory.pkl"), "wb") as handle:
            pickle.dump(payload, handle, pickle.HIGHEST_PROTOCOL)

    def load(self, load_folder, prepend=""):
        with open(os.path.join(load_folder, prepend + "geometry_memory.pkl"), "rb") as handle:
            payload = pickle.load(handle)
        self.geometry_config = payload["geometry_config"]
        self.n_nearest_neighbours = payload["n_nearest_neighbours"]
        self.fit(payload["features"], payload["bin_ids"])
