...
@click.option("--image_scores_path", type=str, default="", help="Path to save per-image scores CSV (overrides default results folder)")
def main(**kwargs):
    pass


@main.result_callback()
def run(methods, results_path, gpu, seed, save_segmentation_images, save_image_scores, image_scores_path):
    ...
    for dataloader_count, dataloaders in enumerate(dataloader_iter):
        ...
        with device_context:
            ...
            if save_image_scores:
                ...
                image_score_df = pd.DataFrame({'image':image_names, 'image_score_mean':scores.tolist()})
                if image_scores_path:
                    out_path = image_scores_path
                else:
                    out_path = os.path.join(results_path, f"image_scores_{dataset_name}.csv")
                os.makedirs(os.path.dirname(out_path) or results_path, exist_ok=True)
